...
function getTvShow(idGenero, idProvider) {
  ...
  let url = "https://api.themoviedb.org/3/discover/tv?api_key=4786cbeb3613782a4448fef983384b30&language=es-ES&watch_region=ES";
  if (idGenero) {
  ...
  fetch(url, requestOptions)
    ...
    .then((result) => {
      document.getElementById("tvshow").innerHTML = "";
      for (const show of result.results) {
        document.getElementById("tvshow").innerHTML += ` <div class="bg-white rounded-lg shadow-lg">
      
              <img class="h-[277px] w-full object-cover rounded-t-lg" src="https://image.tmdb.org/t/p/w500${show.poster_path}" alt="">
      
              <div class="p-4">
                  <a class="font-semibold hover:text-sky-500" href="#">${show.name}</a>
                  <p class="text-slate-500">${show.first_air_date}</p>
              </div>
...
function getGenres() {
  ...
  fetch("https://api.themoviedb.org/3/genre/tv/list?api_key=4786cbeb3613782a4448fef983384b30&language=es", requestOptions)
    .then((response) => response.json())
    .then((result) => {
...
function handleButtonClick(idGenero, providerId, buttonId) {
  ...
  if (isButtonClicked) {
    ...
    getTvShow(); // Llamar a la función getTvShow sin parámetros
  } else {
    buttonElement.classList.add("border-sky-500");
    getTvShow(idGenero, providerId); // Llamar a la función getTvShow con parámetros
  }
}
